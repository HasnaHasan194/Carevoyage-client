...
import { useAgencySalesReport } from "@/hooks/agency/useAgencySalesReport";
import { agencyApi, type GetSalesReportParams } from "@/services/agency/agencyService";
...
export const AgencySalesReportPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useAgencySalesReport(params);

  const handleExportPdf = async () => {
    setExporting("pdf");
    try {
      const blob = await agencyApi.exportSalesReportPdf(params);
      const name = `agency-sales-report-${params?.startDate ?? "all"}-${params?.endDate ?? "all"}.pdf`;
  ...
  const handleExportExcel = async () => {
    setExporting("excel");
    try {
      const blob = await agencyApi.exportSalesReportExcel(params);
      const name = `agency-sales-report-${params?.startDate ?? "all"}-${params?.endDate ?? "all"}.xlsx`;
      downloadBlob(blob, name);
    ...
    >
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <p className="text-sm" style={{ color: CREAM.muted }}>
          Your agency sales performance. Filter by date range and export to PDF or Excel.
        </p>
